...
A local browser-based app that converts documents to Markdown with one-click simplicity, handles password-protected PDFs securely, remembers PDF passwords in the system keychain, and keeps a metadata-only audit trail. Nothing leaves the machine.

## Core user

Solo professional on macOS who regularly converts documents to Markdown for note-taking, archival, or downstream processing. Primary use case: monthly bank statement PDFs with known passwords.

## Core workflow

```
1. Open app in browser (localhost)
2. Upload file(s) or paste YouTube URL
3. If PDF is encrypted:
...
4. Preview Markdown output
5. Download .md file (or batch ZIP)
6. Metadata logged to history (no content stored)
7. Temp files wiped
```
